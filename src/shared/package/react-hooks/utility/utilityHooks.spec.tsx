...
import { useIsFirstRender, useIsomorphicLayoutEffect, useSsr } from '@/shared/package/react-hooks';
...
describe('useSsr hook testing', () => {
	it('Should return object, isServer property boolean, isBrowser object', () => {
		const { result } = renderHook(() => useSsr());
		const { isBrowser, isServer } = result.current;
		expect(typeof result.current).toBe('object');
		expect(typeof isServer).toBe('boolean');
		expect(typeof isBrowser).toBe('object');
	});
});
